...
def HexToColor(s):
    r,g,b = s.split(' ')
    return Colour(red=int(r,16), green=int(g,16), blue=int(b,16))
...
def setwxconfigfiledefaults():
    CHECKINGCOLOR = ColorToHex(SystemSettings_GetColour(SYS_COLOUR_3DSHADOW)) 	 
    DOWNLOADCOLOR = ColorToHex(SystemSettings_GetColour(SYS_COLOUR_ACTIVECAPTION))
...
class configReader:
    ...
    def __init__(self):
        self.configfile = Config("BitTorrent",style=CONFIG_USE_LOCAL_FILE)
    ...
    def importOldGUIConfig(self):
        oldconfig = Config("BitTorrent",style=CONFIG_USE_LOCAL_FILE)
...
#        oldconfig.DeleteAll()
    ...
    def setColorIcon(self, xxicon, xxiconptr, xxcolor):
        ...
        idata.SetBrush(Brush(xxcolor,SOLID))
        idata.DrawRectangle(0,0,16,16)
        idata.SelectObject(NullBitmap)
        xxiconptr.Refresh()
    ...
    def configMenu(self, parent):
      ...
      try:
        ...
        colsizer = FlexGridSizer(cols = 1, vgap = 8)

        self.gui_stretchwindow_checkbox = CheckBox(panel, -1, "Stretch window to fit torrent name *")
        ...
        self.gui_displaystats_checkbox = CheckBox(panel, -1, "Display peer and seed statistics")
        ...
        self.gui_displaymiscstats_checkbox = CheckBox(panel, -1, "Display miscellaneous other statistics")
        ...
        self.buffering_checkbox = CheckBox(panel, -1, "Enable read/write buffering *")
        self.buffering_checkbox.SetFont(self.default_font)
        self.buffering_checkbox.SetValue(self.config['buffer_reads'])

        self.breakup_checkbox = CheckBox(panel, -1, "Break-up seed bitfield to foil ISP manipulation")
        self.breakup_checkbox.SetFont(self.default_font)
        self.breakup_checkbox.SetValue(self.config['breakup_seed_bitfield'])

        self.autoflush_checkbox = CheckBox(panel, -1, "Flush data to disk every 5 minutes")
        ...
        if sys.version_info >= (2,3) and socket.has_ipv6:
            self.ipv6enabled_checkbox = CheckBox(panel, -1, "Initiate and receive connections via IPv6 *")
            self.ipv6enabled_checkbox.SetFont(self.default_font)
            self.ipv6enabled_checkbox.SetValue(self.config['ipv6_enabled'])

        self.gui_forcegreenonfirewall_checkbox = CheckBox(panel, -1,
        ...
        cryptoButton = Button(panel, -1, 'Encryption/Security Settings...')

        self.minport_data = SpinCtrl(panel, -1, '', (-1,-1), (self.FONT*8, -1))
        ...
        self.maxport_data = SpinCtrl(panel, -1, '', (-1,-1), (self.FONT*8, -1))
        ...
        self.gui_ratesettingsdefault_data=Choice(panel, -1, choices = ratesettingslist)
        ...
        self.maxdownload_data = SpinCtrl(panel, -1, '', (-1,-1), (self.FONT*7, -1))
        ...
        self.gui_ratesettingsmode_data=RadioBox(panel, -1, 'Rate Settings Mode',
        ...
        if (sys.platform == 'win32'):
            self.win32_taskbar_icon_checkbox = CheckBox(panel, -1, "Minimize to system tray")
        ...
        self.gui_default_savedir_ctrl = TextCtrl(parent = panel, id = -1, 
                            value = self.config['gui_default_savedir'],        
                            size = (26*self.FONT, -1), style = TE_PROCESS_TAB)
        self.gui_default_savedir_ctrl.SetFont(self.default_font)

        self.gui_savemode_data=RadioBox(panel, -1, 'Ask where to save: *',
        ...
        self.checkingcolor_icon = EmptyBitmap(16,16)
        self.checkingcolor_iconptr = StaticBitmap(panel, -1, self.checkingcolor_icon)
        self.setColorIcon(self.checkingcolor_icon, self.checkingcolor_iconptr, self.checkingcolor)

        self.downloadcolor_icon = EmptyBitmap(16,16)
        self.downloadcolor_iconptr = StaticBitmap(panel, -1, self.downloadcolor_icon)
        self.setColorIcon(self.downloadcolor_icon, self.downloadcolor_iconptr, self.downloadcolor)

        self.seedingcolor_icon = EmptyBitmap(16,16)
        self.seedingcolor_iconptr = StaticBitmap(panel, -1, self.seedingcolor_icon)
        self.setColorIcon(self.seedingcolor_icon, self.downloadcolor_iconptr, self.seedingcolor)
        
        rowsizer = FlexGridSizer(cols = 2, hgap = 20)

        block12sizer = FlexGridSizer(cols = 1, vgap = 12)

        block1sizer = FlexGridSizer(cols = 1, vgap = 2)
        if (sys.platform == 'win32'):
        ...
        block12sizer.Add(cryptoButton, 0, ALIGN_CENTER)

        colorsizer = StaticBoxSizer(StaticBox(panel, -1, "Gauge Colors:"), VERTICAL)
        colorsizer1 = FlexGridSizer(cols = 7)
        colorsizer1.Add(wxStaticText('           Checking: '), 1, ALIGN_BOTTOM)
        colorsizer1.Add(self.checkingcolor_iconptr, 1, ALIGN_BOTTOM)
        colorsizer1.Add(wxStaticText('   Downloading: '), 1, ALIGN_BOTTOM)
        colorsizer1.Add(self.downloadcolor_iconptr, 1, ALIGN_BOTTOM)
        colorsizer1.Add(wxStaticText('   Seeding: '), 1, ALIGN_BOTTOM)
        colorsizer1.Add(self.seedingcolor_iconptr, 1, ALIGN_BOTTOM)
        colorsizer1.Add(wxStaticText('  '))
        ...
        block12sizer.Add(colorsizer, 1, ALIGN_LEFT)

        rowsizer.Add(block12sizer)

        block3sizer = FlexGridSizer(cols = 1)

        portsettingsSizer = StaticBoxSizer(StaticBox(panel, -1, "Port Range:*"), VERTICAL)
        portsettingsSizer1 = GridSizer(cols = 2, vgap = 1)
        portsettingsSizer1.Add(wxStaticText('From: '), 1, ALIGN_CENTER_VERTICAL|ALIGN_RIGHT)
        portsettingsSizer1.Add(self.minport_data, 1, ALIGN_BOTTOM)
        portsettingsSizer1.Add(wxStaticText('To: '), 1, ALIGN_CENTER_VERTICAL|ALIGN_RIGHT)
        portsettingsSizer1.Add(self.maxport_data, 1, ALIGN_BOTTOM)
        portsettingsSizer.Add(portsettingsSizer1)
        portsettingsSizer.Add(self.randomport_checkbox, 1, ALIGN_CENTER)
        block3sizer.Add(portsettingsSizer, 1, ALIGN_CENTER)
        block3sizer.Add(wxStaticText(' '))
        block3sizer.Add(self.gui_ratesettingsmode_data, 1, ALIGN_CENTER)
        block3sizer.Add(wxStaticText(' '))
        ratesettingsSizer = FlexGridSizer(cols = 1, vgap = 2)
        ratesettingsSizer.Add(wxStaticText('Default Rate Setting: *'), 1, ALIGN_CENTER)
        ratesettingsSizer.Add(self.gui_ratesettingsdefault_data, 1, ALIGN_CENTER)
        block3sizer.Add(ratesettingsSizer, 1, ALIGN_CENTER)
        if (sys.platform == 'win32'):
            block3sizer.Add(wxStaticText(' '))
            upnpSizer = FlexGridSizer(cols = 1, vgap = 2)
            upnpSizer.Add(wxStaticText('UPnP Port Forwarding: *'), 1, ALIGN_CENTER)
            upnpSizer.Add(self.upnp_data, 1, ALIGN_CENTER)
            block3sizer.Add(upnpSizer, 1, ALIGN_CENTER)
        
        rowsizer.Add(block3sizer)
        colsizer.Add(rowsizer)

        block4sizer = FlexGridSizer(cols = 3, hgap = 15)        
        savepathsizer = FlexGridSizer(cols = 2, vgap = 1)
        savepathsizer.Add(wxStaticText('Default Save Path: *'))
        savepathsizer.Add(wxStaticText(' '))
        savepathsizer.Add(self.gui_default_savedir_ctrl, 1, EXPAND)
        savepathButton = Button(panel, -1, '...', size = (18,18))
#        savepathButton.SetFont(self.default_font)
        savepathsizer.Add(savepathButton, 0, ALIGN_CENTER)
        savepathsizer.Add(self.gui_savemode_data, 0, ALIGN_CENTER)
        block4sizer.Add(savepathsizer, -1, ALIGN_BOTTOM)

        fontsizer = FlexGridSizer(cols = 1, vgap = 2)
        fontsizer.Add(wxStaticText(''))
        fontsizer.Add(wxStaticText('Font: *'), 1, ALIGN_CENTER)
        fontsizer.Add(self.gui_font_data, 1, ALIGN_CENTER)
        block4sizer.Add(fontsizer, 1, ALIGN_CENTER_VERTICAL)

        dratesettingsSizer = FlexGridSizer(cols = 1, vgap = 2)
        dratesettingsSizer.Add(wxStaticText('Default Max'), 1, ALIGN_CENTER)
        dratesettingsSizer.Add(wxStaticText('Download Rate'), 1, ALIGN_CENTER)
        dratesettingsSizer.Add(wxStaticText('(kB/s): *'), 1, ALIGN_CENTER)
        dratesettingsSizer.Add(self.maxdownload_data, 1, ALIGN_CENTER)
        dratesettingsSizer.Add(wxStaticText('(0 = disabled)'), 1, ALIGN_CENTER)
        
        block4sizer.Add(dratesettingsSizer, 1, ALIGN_CENTER_VERTICAL)

        colsizer.Add(block4sizer, 0, ALIGN_CENTER)

        savesizer = GridSizer(cols = 4, hgap = 10)
        saveButton = Button(panel, -1, 'Save')
#        saveButton.SetFont(self.default_font)
        savesizer.Add(saveButton, 0, ALIGN_CENTER)

        cancelButton = Button(panel, -1, 'Cancel')
#        cancelButton.SetFont(self.default_font)
        savesizer.Add(cancelButton, 0, ALIGN_CENTER)

        defaultsButton = Button(panel, -1, 'Revert to Defaults')
#        defaultsButton.SetFont(self.default_font)
        savesizer.Add(defaultsButton, 0, ALIGN_CENTER)

        advancedButton = Button(panel, -1, 'Advanced...')
#        advancedButton.SetFont(self.default_font)
        savesizer.Add(advancedButton, 0, ALIGN_CENTER)
        colsizer.Add(savesizer, 1, ALIGN_CENTER)

        resizewarningtext=wxStaticText('* These settings will not take effect until the next time you start BitTorrent', self.FONT-2)
        colsizer.Add(resizewarningtext, 1, ALIGN_CENTER)
        ...
        def savepath_set(evt, self = self):
          try:
            ...
            dl = DirDialog(self.panel, 'Choose a default directory to save to', 
                d, style = DD_DEFAULT_STYLE | DD_NEW_DIR_BUTTON)
    ...
    def advancedMenu(self, event = None):
      try:
        ...
        self.advancedMenuBox = Frame(None, -1, 'BitTornado Advanced Preferences', size = (1,1),
                            style = DEFAULT_FRAME_STYLE|FULL_REPAINT_ON_RESIZE)
        if (sys.platform == 'win32'):
            self.advancedMenuBox.SetIcon(self.icon)

        panel = Panel(self.advancedMenuBox, -1)
        ...
        colsizer = FlexGridSizer(cols = 1, hgap = 13, vgap = 13)
        warningtext = wxStaticText('CHANGE THESE SETTINGS AT YOUR OWN RISK', self.FONT+4, True, 'Red')
        colsizer.Add(warningtext, 1, ALIGN_CENTER)

        self.ip_data = TextCtrl(parent = panel, id = -1, 
                    value = self.advancedConfig['ip'],
                    size = (self.FONT*13, int(self.FONT*2.2)), style = TE_PROCESS_TAB)
        self.ip_data.SetFont(self.default_font)
        
        self.bind_data = TextCtrl(parent = panel, id = -1, 
                    value = self.advancedConfig['bind'],
                    size = (self.FONT*13, int(self.FONT*2.2)), style = TE_PROCESS_TAB)
        self.bind_data.SetFont(self.default_font)
        
        if sys.version_info >= (2,3) and socket.has_ipv6:
            self.ipv6bindsv4_data=Choice(panel, -1,
        ...
        self.minpeers_data = SpinCtrl(panel, -1, '', (-1,-1), (self.FONT*7, -1))
        ...
        self.displayinterval_data = SpinCtrl(panel, -1, '', (-1,-1), (self.FONT*7, -1))
        ...
        self.alloctype_data=Choice(panel, -1,
        ...
        self.allocrate_data = SpinCtrl(panel, -1, '', (-1,-1), (self.FONT*7,-1))
        ...
        self.locking_data=Choice(panel, -1,
        ...
        self.doublecheck_data=Choice(panel, -1,
        ...
        self.maxfilesopen_data=Choice(panel, -1, choices = self.maxfilesopen_choices)
        ...
        self.maxconnections_data=Choice(panel, -1, choices = self.maxconnections_choices)
        ...
        self.superseeder_data=Choice(panel, -1,
        ...
        self.expirecache_data=Choice(panel, -1, choices = self.expirecache_choices)
        ...
        twocolsizer = FlexGridSizer(cols = 2, hgap = 20)
        datasizer = FlexGridSizer(cols = 2, vgap = 2)
        datasizer.Add(wxStaticText('Local IP: '), 1, ALIGN_CENTER_VERTICAL)
        datasizer.Add(self.ip_data)
        datasizer.Add(wxStaticText('IP to bind to: '), 1, ALIGN_CENTER_VERTICAL)
        datasizer.Add(self.bind_data)
        if sys.version_info >= (2,3) and socket.has_ipv6:
            datasizer.Add(wxStaticText('IPv6 socket handling: '), 1, ALIGN_CENTER_VERTICAL)
            datasizer.Add(self.ipv6bindsv4_data)
        datasizer.Add(wxStaticText('Minimum number of peers: '), 1, ALIGN_CENTER_VERTICAL)
        datasizer.Add(self.minpeers_data)
        datasizer.Add(wxStaticText('Display interval (ms): '), 1, ALIGN_CENTER_VERTICAL)
        datasizer.Add(self.displayinterval_data)
        datasizer.Add(wxStaticText('Disk allocation type:'), 1, ALIGN_CENTER_VERTICAL)
        datasizer.Add(self.alloctype_data)
        datasizer.Add(wxStaticText('Allocation rate (MiB/s):'), 1, ALIGN_CENTER_VERTICAL)
        datasizer.Add(self.allocrate_data)
        datasizer.Add(wxStaticText('File locking:'), 1, ALIGN_CENTER_VERTICAL)
        datasizer.Add(self.locking_data)
        datasizer.Add(wxStaticText('Extra data checking:'), 1, ALIGN_CENTER_VERTICAL)
        datasizer.Add(self.doublecheck_data)
        datasizer.Add(wxStaticText('Max files open:'), 1, ALIGN_CENTER_VERTICAL)
        datasizer.Add(self.maxfilesopen_data)
        datasizer.Add(wxStaticText('Max peer connections:'), 1, ALIGN_CENTER_VERTICAL)
        datasizer.Add(self.maxconnections_data)
        datasizer.Add(wxStaticText('Default seeding mode:'), 1, ALIGN_CENTER_VERTICAL)
        datasizer.Add(self.superseeder_data)
        datasizer.Add(wxStaticText('Expire resume data(days):'), 1, ALIGN_CENTER_VERTICAL)
        datasizer.Add(self.expirecache_data)
        
        twocolsizer.Add(datasizer)

        infosizer = FlexGridSizer(cols = 1)
        self.hinttext = wxStaticText('', self.FONT, False, 'Blue')
        infosizer.Add(self.hinttext, 1, ALIGN_LEFT|ALIGN_CENTER_VERTICAL)
        infosizer.SetMinSize((180,100))
        twocolsizer.Add(infosizer, 1, EXPAND)

        colsizer.Add(twocolsizer)

        savesizer = GridSizer(cols = 3, hgap = 20)
        okButton = Button(panel, -1, 'OK')
#        okButton.SetFont(self.default_font)
        savesizer.Add(okButton, 0, ALIGN_CENTER)
...
#        defaultsButton.SetFont(self.default_font)
        ...
        resizewarningtext=wxStaticText('None of these settings will take effect until the next time you start BitTorrent', self.FONT-2)
        colsizer.Add(resizewarningtext, 1, ALIGN_CENTER)
    ...
    def cryptoMenu(self, event = None):
      try:
        ...
        self.cryptoMenuBox = Frame(None, -1, 'BitTornado Encryption/Security Preferences', size = (1,1),
                            style = DEFAULT_FRAME_STYLE|FULL_REPAINT_ON_RESIZE)
        if (sys.platform == 'win32'):
            self.cryptoMenuBox.SetIcon(self.icon)

        panel = Panel(self.cryptoMenuBox, -1)
#        self.panel = panel
        ...
        self.cryptomode_data=RadioBox(panel, -1, 'Encryption',
                style = RA_SPECIFY_COLS, majorDimension = 1,
                choices = [
        ...
        self.security_checkbox = CheckBox(panel, -1, "Don't allow multiple connections from the same IP")
        self.security_checkbox.SetFont(self.default_font)
        self.security_checkbox.SetValue(self.cryptoConfig['security'])

        self.autokick_checkbox = CheckBox(panel, -1, "Kick/ban clients that send you bad data")
        ...
        block2sizer = FlexGridSizer(cols = 1, vgap = 2)
